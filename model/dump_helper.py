...
def dump_results(end_points, dump_dir, config, inference_switch=False):
    ...
    idx_beg = 0
    #-------------------------------------------------------------------------------------
    semantic_lable = torch.argmax(end_points['sem_cls_scores'], -1)# B,num_proposal
    semantic_lable = semantic_lable.detach().cpu().numpy()
    for i in range(batch_size):
        ...
        if np.sum(objectness_prob>DUMP_CONF_THRESH)>0:
            ...
            if len(obbs)>0:
                ...
                pc_util.write_oriented_bbox_color(obbs[np.logical_and(objectness_prob>DUMP_CONF_THRESH, pred_mask[i,:]==1),:], semantic_lable[i, pred_mask[i,:]==1], os.path.join(dump_dir, '%06d_pred_confident_nms_bbox.ply'%(idx_beg+i)))
                print('------------------------------------------', semantic_lable[i, pred_mask[i,:]==1])
                #------------------------------------------------
                pc_util.write_oriented_bbox_color(obbs[pred_mask[i,:]==1,:], semantic_lable[i, pred_mask[i,:]==1], os.path.join(dump_dir, '%06d_pred_nms_bbox.ply'%(idx_beg+i)))
    ...
    semantic_label = end_points['sem_cls_label'].cpu().numpy()
    ...
    for i in range(batch_size):
        ...
        if len(obbs)>0:
            ...
            print('+++++++++++++++++++++++++++++++++++++++++++', semantic_label[i, :])
            pc_util.write_oriented_bbox_color(obbs, semantic_label[i, :], os.path.join(dump_dir, '%06d_gt_bbox.ply'%(idx_beg+i)))
