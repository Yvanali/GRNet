...
class ProposalModule(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling, seed_feat_dim=256):
        ...
        self.conv11 = torch.nn.Conv1d(128,128,1)
        self.conv22 = torch.nn.Conv1d(128,128,1)
        self.conv33 = torch.nn.Conv1d(128,2+3+num_heading_bin*2+num_size_cluster*4+self.num_class,1)
    ...
    def forward(self, xyz, features, end_points):
        ...
        center = xyz + net_transposed[:,:,2:5]

        _, idx = pointnet2_utils.three_nn(center, center)
        B,C,N = features.shape
        grouped_features = pointnet2_utils.grouping_operation(features, idx) # (B, C, npoint, nsample)
        atten = Variable(torch.zeros([1,1,1,1]), requires_grad=True).expand(B,N,3,C).cuda()
        grouped_features = grouped_features.permute(0,2,3,1) # (B, mlp[-1], npoint, nsample) 
        #atten = self.mlp_module(grouped_features0)  # (B, mlp[-1], npoint, nsample)
        #atten = grouped_features     
        atten = self.leakyrelu(grouped_features*atten)
        atten = F.softmax(atten,dim=2)  # (B, npoint, nsample,mlp[-1])              
        feat_rela = torch.sum(atten*grouped_features,dim=2)#(b,n,c)
        feat_rela = feat_rela.transpose(2,1)
        features = features + feat_rela

        # --------- PROPOSAL GENERATION ---------
        net1 = F.relu(self.bn11(self.conv11(features))) 
        net1 = F.relu(self.bn22(self.conv2(net1))) 
        net1 = self.conv33(net1) # (batch_size, 2+3+num_heading_bin*2+num_size_cluster*4, num_proposal)
        net = net+net1
