...
class GeoPointSAModule(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        taylor = Variable(torch.ones(shape), requires_grad=True).expand(B,self.npoint, self.nsample,8).cuda()
        basis_taylor = Variable(torch.zeros(shape), requires_grad=True).expand(B,self.npoint, self.nsample,1).cuda()

        g1 = taylor[:, :, :, 0] * X + taylor[:, :, :, 1] * Y + taylor[:, :, :, 2] * Z + basis_taylor[:, :, :, 0]
        g2 = taylor[:, :, :, 3] * X*Y + taylor[:, :, :, 4] * Y*Z + taylor[:, :, :, 5] * Z*X + taylor[:, :, :, 6]*X*Y*Z
        g_d = torch.exp(g1+g2)

        g_d = g_d.view(B, self.npoint, self.nsample, 1).contiguous()
        grouped_features1 = grouped_features.permute(0,2,3,1).contiguous()#(B, self.npoint, self.nsample, C)
        grouped_features1 = grouped_features1*g_d
        grouped_features1 = grouped_features1.permute(0,3,1,2)
        grouped_features = grouped_features+grouped_features1
