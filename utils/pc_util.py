...
def write_oriented_bbox(scene_bbox, out_filename):
    ...
    def convert_oriented_box_to_trimesh_fmt(box):
        ...
        box_trimesh_fmt = trimesh.creation.box(lengths, trns)
        box_trimesh_fmt.visual.vertex_colors = trimesh.visual.random_color()  
        return box_trimesh_fmt
...
def write_oriented_bbox_color(scene_bbox, lables,out_filename):
    """Export oriented (around Z axis) scene bbox to meshes
    Args:
        scene_bbox: (N x 7 numpy array): xyz pos of center and 3 lengths (dx,dy,dz)
            and heading angle around Z axis.
    ...
    def convert_oriented_box_to_trimesh_fmt(box, nclass):
        ctr = box[:3]
        lengths = box[3:6]
        trns = np.eye(4)
        trns[0:3, 3] = ctr
        ...
        trns[0:3,0:3] = heading2rotmat(box[6])
        box_trimesh_fmt = trimesh.creation.box(lengths, trns)
        box_trimesh_fmt.visual.vertex_colors = color[nclass]#[2, 2, 192, 255]
        #label = lables.astype(int)
        #box_trimesh_fmt.visual.vertex_colors = color[label,:]#[255*label/num_classes, 255*label/num_classes,255*label/num_classes,255]
        return box_trimesh_fmt

    #color map
    color = [[252, 2, 192, 255],[196, 252, 2, 255], [252, 2, 221, 255],[191, 2, 252, 255],[101, 252, 2, 255],[252, 2, 15, 255],[252, 154, 2, 255],
             [252, 60,2, 255], [2, 252, 112, 255], [171, 2, 252, 255], [252,60, 130, 255], [10, 10, 252, 255],[ 64, 2, 252, 255],[90, 130, 252, 255],
             [252, 182, 40, 255], [2, 149, 252,255],[50,252,160,255],[30, 40, 252, 255],[180, 157, 140, 255],[100, 14, 177,255],[252, 159,2, 255] ]
    ...
    for box in scene_bbox:
        nclass = label[i]
        scene.add_geometry(convert_oriented_box_to_trimesh_fmt(box, nclass))
        i = i+1
...
def write_lines_as_cylinders(pcl, filename, rad=0.005, res=64):
    ...
    mesh_list = trimesh.util.concatenate(scene.dump())
    trimesh.io.export.export_mesh(mesh_list, '{filename}.ply', file_type='ply')
